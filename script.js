...
function initHours() {
    ...
    if (d === todayDay) {
      ...
      if (h) {
        ...
        if (nowMins >= openMins && nowMins < closeMins) {
          ...
          if (badge) {
            badge.textContent = 'Právě otevřeno';
            badge.className = 'hours-badge open-now';
          }
        }
      }
    }
  ...
  if (!h) {
    icon = '🔒';
    statusHTML = `<div class="hours-today-status is-closed">Dnes zavřeno — uvidíme se zítra!</div>`;
  } else {
    const [oh, om] = h.open.split(':').map(Number);
    const [ch, cm] = h.close.split(':').map(Number);
    const openMins = oh * 60 + om;
    ...
    if (nowMins < openMins) {
      icon = '⏳';
      statusHTML = `<div class="hours-today-status">Otevíráme dnes v <strong>${h.open}</strong> — těšíme se!</div>`;
    } else if (nowMins >= openMins && nowMins < closeMins) {
      icon = '✅';
      const hLeft = Math.floor(minsLeft / 60);
      const mLeft = minsLeft % 60;
      const timeStr = hLeft > 0 ? `${hLeft}h ${mLeft}min` : `${mLeft} min`;
      statusHTML = `<div class="hours-today-status is-open">Právě otevřeno · Zavíráme v ${h.close} (za ${timeStr})</div>`;
    } else {
      icon = '🔒';
      statusHTML = `<div class="hours-today-status is-closed">Dnes už zavřeno · Byli jsme tu ${h.open}–${h.close}</div>`;
    }
  }

  todayEl.innerHTML = `
    <div class="hours-today-icon">${icon}</div>
    <div class="hours-today-info">
      <div class="hours-today-label">Dnes · ${todayName}</div>
      ${statusHTML}
    </div>
  `;
}
