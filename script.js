...
// ===== HOURS MODAL TOGGLE =====
document.getElementById('hoursToggle').addEventListener('click', openHours);
document.getElementById('hoursClose').addEventListener('click', closeHours);
document.getElementById('hoursOverlay').addEventListener('click', closeHours);
...
const HOURS = {
  ...
  1: { open: '8:00',  close: '17:00' },
  2: { open: '7:00',  close: '16:00' },
  3: { open: '9:00',  close: '18:00' },
  4: { open: '8:00',  close: '12:00' },
  5: { open: '6:00',  close: '20:00' },
  6: { open: '10:00', close: '12:00' },
};
...
function initHours() {
  ...
  if (h) {
    ...
    if (nowMins >= oh*60+om && nowMins < ch*60+cm) isOpenNow = true;
  }

  // Navbar dot
  const dot = document.getElementById('hoursStatusDot');
  dot.className = 'hours-dot ' + (isOpenNow ? 'open-now' : 'closed-now');

  // Build today banner HTML
  let icon, statusHTML;
  if (!h) {
    icon = '🔒';
    statusHTML = `<div class="hmt-status is-closed">Dnes zavřeno — uvidíme se zítra!</div>`;
  } else {
    const [oh, om] = h.open.split(':').map(Number);
    const [ch, cm] = h.close.split(':').map(Number);
    ...
    if (nowMins < openMins) {
      icon = '⏳';
      statusHTML = `<div class="hmt-status">Otevíráme v <strong>${h.open}</strong> — těšíme se!</div>`;
    } else if (isOpenNow) {
      icon = '✅';
      const hL = Math.floor(minsLeft/60), mL = minsLeft%60;
      const timeStr = hL > 0 ? `${hL}h ${mL}min` : `${mL} min`;
      statusHTML = `<div class="hmt-status is-open">Právě otevřeno · Zavíráme v ${h.close} (za ${timeStr})</div>`;
    } else {
      icon = '🔒';
      statusHTML = `<div class="hmt-status is-closed">Dnes už zavřeno · Byli jsme tu ${h.open}–${h.close}</div>`;
    }
  }

  const bannerHTML = `
    <div class="hmt-icon">${icon}</div>
    <div>
      <div class="hmt-label">Dnes · ${todayName}</div>
      ${statusHTML}
    </div>`;

  // Apply to modal
  const modalToday = document.getElementById('hoursModalToday');
  if (modalToday) modalToday.innerHTML = bannerHTML;

  // Apply to page section
  const pageToday = document.getElementById('hoursToday');
  if (pageToday) pageToday.innerHTML = bannerHTML;

  // Highlight today rows — modal rows
  document.querySelectorAll('.hm-row').forEach(row => {
    if (parseInt(row.dataset.day) === todayDay) {
      row.classList.add('today');
      if (isOpenNow) {
        const b = row.querySelector('.hm-badge');
        if (b) { b.textContent = 'Právě otevřeno'; b.className = 'hm-badge open-now'; }
      }
    }
  });

  // Highlight today rows — page section rows
  document.querySelectorAll('.hours-row').forEach(row => {
    if (parseInt(row.dataset.day) === todayDay) {
      row.classList.add('today');
      if (isOpenNow) {
        const b = row.querySelector('.hours-badge');
        if (b) { b.textContent = 'Právě otevřeno'; b.className = 'hours-badge open-now'; }
      }
    }
  });
}
